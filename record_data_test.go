...
func TestRecordData_Encode(t *testing.T) {
	...
	rdBytes, err := testRecordData.Encode()
	if err != nil {
...
func TestRecordData_Decode(t *testing.T) {
	...
	if err := rd.Decode(testRecordDataBytes); err != nil {
		t.Errorf("Ошибка декадирования: %v\n", err)
	}

	if !reflect.DeepEqual(rd, testRecordData) {
		t.Errorf("Запись не совпадают: %v != %v ", rd, testRecordData)
	}
